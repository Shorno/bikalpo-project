...
import { financialLedger, supplier } from "@bikalpo-project/db/schema";
...
const payablePermission = (action: "view" | "create" | "update") =>
  shopOrWarehousePermissionProcedure("shop_payables", action, "purchase");
...
export const supplierPaymentRouter = {
  ...
  paySupplier: payablePermission("update")
    .route({
      method: "POST",
  ...
  getSupplierLedger: payablePermission("view")
    .route({
      method: "POST",
    ...
    .handler(async ({ context, input }) => {
      return db.query.financialLedger.findMany({
        where: and(
          eq(
            financialLedger.ownerId,
            shopOrWarehouseOwnerScope(context.session.user, "purchase").ownerId,
          ),
  ...
  getPayableSummary: payablePermission("view")
    .route({
      method: "POST",
