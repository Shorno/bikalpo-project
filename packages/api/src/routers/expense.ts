...
) => shopOrWarehousePermissionProcedure("shop_expenses", action, "finance");
const financeCategoryPermission = (action: "view" | "create") =>
...
export const expenseRouter = {
  ...
  createExpense: expensePermission("create")
    .route({
      method: "POST",
      path: "/expenses",
    ...
    .handler(async ({ context, input }) => {
      ...
      const expenseNumber = await generateExpenseNumber(
        shopOrWarehouseOwnerScope(context.session.user, "finance").ownerId,
      );
      ...
      const [created] = await db
        ...
        .values({
          ...
          ownerId: shopOrWarehouseOwnerScope(context.session.user, "finance")
            .ownerId,
  ...
  getExpenses: expensePermission("view")
    .route({
      method: "POST",
      path: "/expenses/list",
      tags: ["Expense Management"],
  ...
  getExpenseById: expensePermission("view")
    .route({
      method: "POST",
  ...
  voidExpense: expensePermission("delete")
    .route({
      method: "POST",
      path: "/expenses/void",
      tags: ["Expense Management"],
      ...
      description:
        "Mark expense as voided. Creates an adjustment ledger entry.",
    })
    .input(
      z.object({
        id: z.number().int(),
  ...
  getCategories: financeCategoryPermission("view")
    .route({
      method: "POST",
      ...
      return db.query.expenseCategory.findMany({
        where: (c, { eq: eq2, or, isNull }) =>
          or(
            ...
            eq2(
              c.ownerId,
              shopOrWarehouseOwnerScope(context.session.user, "finance")
                .ownerId,
            ),
            isNull(c.ownerId),
          ),
  ...
  createCategory: financeCategoryPermission("create")
    .route({
      method: "POST",
