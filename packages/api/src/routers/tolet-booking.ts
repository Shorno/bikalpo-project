...
function formatBookingCode(booking: Pick<ToletBookingRequest, "publicNumber">) {
...
function bookingDto(booking: ToletBookingRequest, revealPrices = false) {
  return {
    ...
    listingUpdatedAtAtRequest: booking.listingUpdatedAtAtRequest.toISOString(),
  ...
  listMine: consumerProcedure
    ...
    .handler(async ({ context }) => {
      const rows = await db
        ...
        .where(eq(toletBookingRequest.requesterUserId, context.session.user.id))
        ...
        const [owned] = await tx
          ...
          .where(
            and(
              ...
              eq(toletBookingRequest.requesterUserId, context.session.user.id),
            ),
          )
          .limit(1)
          .for("update");
      ...
      const [relevantListing] = await db
        ...
        .where(
          and(
            ...
            eq(toletUnitListing.status, hasRequestInbox ? "active" : "closed"),
          ),
        )
