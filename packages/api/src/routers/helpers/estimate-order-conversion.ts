...
export async function convertEstimateToB2bOrder(input: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    for (const item of estimateData.items) {
      const target = isWarehouseBuyer
        ? await ensureWarehouseBuyerTargetVariant(tx, {
            warehouseId: estimateData.customerId,
            sourceVariantId: item.variantId!,
          })
        : await ensureShopBuyerTargetVariant(tx, {
            shopId: estimateData.customerId,
            sourceVariantId: item.variantId!,
          });
