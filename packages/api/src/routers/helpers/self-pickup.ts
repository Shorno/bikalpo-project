 ...
 * four-digit code. This is the single owner-scoped entry point used by
 * warehouse and retailer routers.
 */
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      if (existingInvoice.completionOtp !== input.otp) {
        throw new ORPCError("BAD_REQUEST", { message: "Invalid pickup OTP" });
      }
