...
const purchasePermission = (
  action: "view" | "create" | "update" | "approve" | "delete",
) =>
...
async function generatePurchaseNumber(warehouseId: string): Promise<string> {
  ...
  const [result] = await db
    ...
    .where(
      and(
        eq(purchase.warehouseId, warehouseId),
        ilike(purchase.purchaseNumber, `${prefix}%`),
      ),
    );
...
export const purchaseRouter = {
  advanceManualRefund: purchasePermission("update")
    .route({
      method: "POST",
  ...
  cancelManual: purchasePermission("delete")
    .route({
      method: "POST",
  ...
  listManual: purchasePermission("view")
    .route({
      method: "POST",
  ...
  getManualDetail: purchasePermission("view")
    .route({
      method: "POST",
  ...
  saveManualDraft: purchasePermission("create")
    .route({
      method: "POST",
  ...
  confirmManual: purchasePermission("approve")
    .route({
      method: "POST",
  ...
  confirmSavedManual: purchasePermission("approve")
    .route({
      method: "POST",
  ...
  addManualPayment: purchasePermission("update")
    .route({
      method: "POST",
  ...
  create: purchasePermission("create")
    .route({
      method: "POST",
    ...
    .handler(async ({ context, input }) => {
      ...
      await db.insert(financialLedger).values({
        entryType:
          input.paymentType === "credit" ? "purchase_credit" : "purchase_cash",
        amount: String(total),
        direction: "debit",
      ...
      return {
        purchase: created,
        message: `Purchase ${purchaseNumber} created (${input.paymentType})`,
      };
    }),

  /** List purchases */
  list: purchasePermission("view")
    .route({
      method: "POST",
      path: "/purchases/list",
      tags: ["Purchase Management"],
  ...
  getSuppliers: purchasePermission("view")
    .route({
      method: "POST",
      path: "/purchases/suppliers",
      tags: ["Purchase Management"],
      summary: "Get suppliers for purchase",
    })
  ...
  getProducts: purchasePermission("view")
    .route({
      method: "POST",
      path: "/purchases/products",
      tags: ["Purchase Management"],
      summary: "Get products for purchase",
    })
