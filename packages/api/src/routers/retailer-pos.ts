...
function assertExpectedPrices(
  ...
  for (const item of requested) {
    ...
    if (
      current &&
      Math.abs(current.unitPrice - item.expectedUnitPrice) >= 0.005
    ) {
...
export const retailerPosRouter = {
  getBootstrap: shopPermissionProcedure("shop_pos", "view")
    .route({
      method: "GET",
    ...
    .handler(async ({ context }) => {
      ...
      return {
        ...
        shop: {
          ...
          ownerName:
            context.session.user.ownerName || context.session.user.name,
  ...
  getCatalog: shopPermissionProcedure("shop_pos", "view")
    .route({
      method: "GET",
    ...
    .handler(async ({ context, input }) => {
      ...
      const progressivelyFiltered = (stopBefore?: keyof typeof filter) =>
        searched.filter((row) => {
          ...
          if (filter.categoryId && row.categoryId !== filter.categoryId)
            return false;
          if (stopBefore === "subCategoryId") return true;
          if (
            filter.subCategoryId &&
            row.subCategoryId !== filter.subCategoryId
          )
            return false;
          if (stopBefore === "coreProductId") return true;
          if (
            filter.coreProductId &&
            row.coreProductId !== filter.coreProductId
          )
            return false;
      ...
      const unique = <T extends { id: number }>(values: T[]) =>
        [...new Map(values.map((value) => [value.id, value])).values()].sort(
          (a, b) =>
            String((a as T & { name: string }).name).localeCompare(
              String((b as T & { name: string }).name),
            ),
        );
      ...
      return {
        ...
        options: {
          types: unique(
            progressivelyFiltered("typeId").map((row) => ({
              id: row.typeId,
              name: row.typeName,
            })),
          ),
          ...
          brands: unique(
            progressivelyFiltered("brandId")
              ...
              .map((row) => ({
                id: row.brandId as number,
                name: row.brandName,
              })),
          ),
          packs: [
            ...new Set(progressivelyFiltered("pack").map((row) => row.pack)),
          ].sort(),
        },
      };
    }),

  searchCustomers: shopPermissionProcedure("shop_customers", "view")
    .route({
      method: "GET",
    ...
    .input(
      z.object({ search: z.string().trim().max(100).optional() }).optional(),
    )
    .handler(async ({ context, input }) => {
      ...
      for (const customer of onlineCustomers) {
        const normalized =
          normalizePosPhone(customer.phone) ||
          `consumer:${customer.linkedUserId}`;
      ...
      const customers = [...byPhone.values()].filter(
        (customer) =>
          !term ||
  ...
  createCustomer: shopPermissionProcedure("shop_customers", "create")
    .route({
      method: "POST",
      path: "/retailer-pos/customers",
      tags: ["Retailer POS"],
      ...
      if (input.linkedUserId) {
        ...
        if (!relationship) {
          throw new ORPCError("NOT_FOUND", {
            message: "Consumer has no order relationship with this shop",
          });
        }
      }
      return {
        customer: await findOrCreatePosCustomer({
  ...
  getCustomerDetail: shopPermissionProcedure("shop_customers", "view")
    .route({
      method: "GET",
    ...
    .handler(async ({ context, input }) => {
      ...
      if (kind === "pos") {
        ...
        if (!linkedUserId && owned.phone) {
          const relatedConsumers = await db
            ...
            .where(and(eq(order.shopId, shopId), eq(order.orderType, "b2c")));
          linkedUserId =
            relatedConsumers.find(
              (candidate) =>
                normalizePosPhone(candidate.phone) ===
                normalizePosPhone(owned.phone),
            )?.userId ?? null;
        }
        customer = {
      ...
      const payments = customer.id
        ? await db
            ...
            .where(
              and(
                eq(posSale.shopId, shopId),
                eq(posSale.customerId, customer.id),
              ),
            )
  ...
  listHeldCarts: shopPermissionProcedure("shop_pos", "view")
    .route({
      method: "GET",
      path: "/retailer-pos/held-carts",
      tags: ["Retailer POS"],
    })
    .input(z.object({}).optional())
    .handler(async ({ context }) => ({
      carts: await db.query.warehousePosCart.findMany({
        where: and(
          eq(posCart.shopId, shopTenantId(context.session.user)),
  ...
  holdCart: shopPermissionProcedure("shop_pos", "create")
    .route({
      method: "POST",
      path: "/retailer-pos/held-carts",
      tags: ["Retailer POS"],
  ...
  cancelHeldCart: shopPermissionProcedure("shop_pos", "delete")
    .route({
      method: "DELETE",
      path: "/retailer-pos/held-carts/{cartId}",
      tags: ["Retailer POS"],
    })
  ...
  completeSale: shopPermissionProcedure("shop_pos", "create")
    .route({
      method: "POST",
      path: "/retailer-pos/sales",
      tags: ["Retailer POS"],
    })
    .input(
      z.object({
    ...
    .handler(async ({ context, input }) => {
      ...
      if (input.heldCartId) {
        ...
        if (!heldCart) {
          throw new ORPCError("NOT_FOUND", {
            message: "Held Cart not found or already completed",
          });
        }
      }
      const lines = await resolveOwnerPosSaleLines(owner, input.items);
        ...
        throw new ORPCError("BAD_REQUEST", {
          message:
            error instanceof Error
              ? error.message
              : "Named customer required for Due sale",
        });
      }
      if (input.paymentMethod !== "cash" && totals.change > 0) {
        throw new ORPCError("BAD_REQUEST", {
          message: "Non-cash payment cannot exceed the sale total",
        });
      }
      ...
      const result = await db.transaction(async (tx) => {
        ...
        await tx.insert(posSaleItem).values(
          lines.map((line) => ({
            saleId: created.id,
  ...
  getSale: shopPermissionProcedure("shop_sales", "view")
    .route({
      method: "GET",
      path: "/retailer-pos/sales/{saleId}",
      tags: ["Retailer POS"],
    })
    ...
    .handler(async ({ context, input }) => {
      const sale = await db.query.warehousePosSale.findFirst({
        where: and(
          eq(posSale.id, input.saleId),
          saleOwnerCondition(shopTenantId(context.session.user)),
        ),
        with: {
      ...
      if (!sale)
        throw new ORPCError("NOT_FOUND", { message: "POS Sale not found" });
      return {
        sale,
        shop: {
          name: context.session.user.shopName || context.session.user.name,
  ...
  listSales: shopPermissionProcedure("shop_sales", "view")
    .route({
      method: "GET",
      path: "/retailer-pos/sales",
      tags: ["Retailer POS"],
    ...
    .handler(async ({ context, input }) => {
      ...
      const posRows =
        filter.source === "online"
          ? []
          : await db.query.warehousePosSale.findMany({
              where: and(...posConditions),
              orderBy: [desc(posSale.createdAt)],
            });
      ...
      const rows = [
        ...
        .filter((row) => {
          ...
          if (filter.payment !== "all") {
            ...
            if (
              filter.payment !== "due" &&
              row.paymentMethod !== filter.payment &&
              !(
                filter.payment === "bank" &&
  ...
  listReceivables: shopPermissionProcedure("shop_receivables", "view")
    .route({
      method: "GET",
      path: "/retailer-pos/receivables",
      tags: ["Retailer POS"],
    })
    .input(
      z.object({ search: z.string().trim().max(100).optional() }).optional(),
    ...
    .handler(async ({ context, input }) => {
      ...
      const conditions: SQL[] = [
        eq(posSale.shopId, shopTenantId(context.session.user)),
  ...
  collectDue: shopPermissionProcedure("shop_receivables", "update")
    .route({
      method: "POST",
      path: "/retailer-pos/receivables/{saleId}/payments",
      tags: ["Retailer POS"],
    })
    .input(
      z.object({
    ...
    .handler(async ({ context, input }) => {
      ...
      return db.transaction(async (tx) => {
        ...
        if (sale.status !== "completed")
          throw new ORPCError("BAD_REQUEST", {
            message: "Cancelled sales cannot receive payment",
          });
        const due = numeric(sale.due);
        if (due <= 0)
          throw new ORPCError("BAD_REQUEST", {
            message: "This sale has no Outstanding Balance",
          });
        if (input.amount > due)
          throw new ORPCError("BAD_REQUEST", {
            message: `Amount exceeds the Outstanding Balance of BDT ${money(due)}`,
          });
        ...
        await tx
          .update(posSale)
          .set({ paid: money(nextPaid), due: money(nextDue) })
          .where(eq(posSale.id, sale.id));
        return { paid: money(nextPaid), due: money(nextDue), duplicate: false };
      });
    }),

  voidSale: shopPermissionProcedure("shop_sales", "delete")
    .route({
      method: "POST",
      path: "/retailer-pos/sales/{saleId}/void",
      tags: ["Retailer POS"],
    })
    .input(
      z.object({
