  ...
  markPurchaseReceived: shopOwnerProcedure
    ...
    .handler(async ({ context, input }) => {
      ...
      const fullyReceived = receiptRows.every((receipt) => {
        ...
        return (
          receipt.receivedQty >=
          Number(item?.modifiedQty ?? item?.quantity ?? 0)
        );
      });
      ...
      await db.transaction(async (tx) => {
        const claimed = await tx
          ...
          .where(
            and(eq(order.id, input.orderId), eq(order.status, "delivered")),
          )
          .returning({ id: order.id });
        if (claimed.length === 0) {
          throw new ORPCError("BAD_REQUEST", {
            message: "Order receipt status changed while it was being updated",
          });
        }
  ...
  getAccountsPayableReport: shopOwnerProcedure
    ...
    .handler(async ({ context, input }) => {
      ...
      const platformRows = bills.map((bill) => {
        ...
        const supplier =
          (bill.warehouseId ? warehouseNames.get(bill.warehouseId) : null) ??
          "Admin";
      ...
      return {
        ...
        summary: rows.reduce(
          (summary, row) => ({
            ...
            overdue: summary.overdue + (row.status === "Overdue" ? row.due : 0),
  ...
  placeWarehouseOrder: shopOwnerProcedure
    ...
    .handler(async ({ context, input }) => {
      ...
      for (const item of input.items) {
        ...
        unitPrice = Math.max(
          0,
          Number(newUnitPrice) - exchangeCreditAmount,
        ).toFixed(2);
      ...
      let checkoutResult: Awaited<
        ReturnType<typeof buildWholesaleCheckoutQuote>
      >;
      try {
        ...
        offers.map(async (offer) => {
          const items = await db
            .select({
              ...
              offerExchangeCreditAmount: openOrderBidItem.exchangeCreditAmount,
              exchangeEnabled: productVariant.exchangeEnabled,
              currentExchangeCreditAmount: productVariant.exchangeCreditAmount,
            ...
            .innerJoin(
              productVariant,
              eq(productVariant.id, inventory.variantId),
            )
          ...
          return {
            ...
            items: items.map((item) => {
              ...
              let liveCylinderSale: ReturnType<
              ...
              return {
                ...
                currentEffectivePrice: Number(
                  liveCylinderSale.effectiveUnitPrice,
                ),
                currentExchangeCreditAmount: Number(
                  liveCylinderSale.exchangeCreditAmount,
                ),
                offerNewUnitPrice:
