...
import {
  ...
  receiveB2bInvoiceShipment,
} from "./helpers/b2b-conversion";
import {
    ...
    .handler(async ({ input }) => {
      ...
      return {
        ...
        checkoutConfiguration: {
          ...
          wholesaleCreditDays: checkoutConfiguration?.wholesaleCreditDays ?? 0,
        },
      };
    }),
        ...
        return {
          ...
          variants: items.map((inv) => {
            ...
            return {
              ...
              canExchange: warehouseVariantAllowsCylinderExchange(inv.variant),
              variant: inv.variant,
            };
          }),
        };
    ...
    .handler(async ({ input }) => {
      ...
      const variants = foundProduct.variants
        .map((variant) => {
          ...
          const retailPrice =
            Number(inventoryRow.retailPrice ?? variant.price) || 0;
      ...
      return {
        ...
        product: {
          ...
          lowestPrice: Math.min(
            ...variants.map((variant) => variant.retailPrice),
          ),
          category: {
      ...
      const shipments = invoices.map((inv) => {
        ...
        return {
          ...
          delivery: link
            ...
            : null,
          otp,
          canReceive: inv.deliveryStatus === "delivered" && !inv.receivedAt,
        };
      });

      const orderTimeline = buildBuyerOrderTimeline(
        result,
        invoices.length > 0,
      );
...
const supplierQueries = {
  ...
  recordSupplierBill: warehouseProcedure
    .route({
      method: "POST",
      path: "/warehouse/suppliers/record-bill",
      tags: ["Warehouse"],
      summary: "Record warehouse supplier bill payable",
    })
    .input(
      z.object({
