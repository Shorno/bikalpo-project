      ...
      const [rows, countResult] = await Promise.all([
        db.query.product.findMany({
          ...
          orderBy: (p, { asc, desc: descFn }) =>
            sort === "oldest" ? [asc(p.createdAt)] : [descFn(p.createdAt)],
          offset,
          limit,
          columns: {
            id: true,
    ...
    .handler(async ({ input }) => {
      ...
      const products = await db.query.product.findMany({
        ...
        orderBy: (p, { asc: ascFn, desc: descFn }) =>
          sort === "oldest" ? [ascFn(p.createdAt)] : [descFn(p.createdAt)],
        limit: 100_000,
        columns: {
          id: true,
          name: true,
  ...
  exportStockPDF: adminProcedure
    ...
    .handler(async ({ input }) => {
      ...
      const drawHeader = (page: typeof currentPage, y: number) => {
        ...
        page.drawLine({
          start: { x: leftMargin, y: y - 6 },
          end: { x: rightMargin, y: y - 6 },
          thickness: 0.5,
      ...
      for (const p of products) {
        ...
        const cells: [string, number][] = [
          [`PRD-${p.id}`, col.id],
          [truncate(p.name, 18), col.name],
          [truncate(p.sku ?? p.slug ?? "", 12), col.sku],
          [truncate(cat + sub, 14), col.category],
          ["0", col.stock],
          [String(p.reorderLevel), col.reorder],
          [formatPrice(p.price), col.price],
          [p.inStock ? "Yes" : "No", col.inStock],
        ];
