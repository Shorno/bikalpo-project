...
- Store header, footer and brand section reuse the same query cache. The section follows the product catalog and matches the public homepage's Browse brands card grid: tinted background, centered logo area, bordered white cards and names below. It uses two columns on mobile, three on small screens, four on medium screens and six on desktop. Cards remain informational; no brand filtering is introduced. Missing/failed logos use the same building icon as the public page while preserving the brand name.
