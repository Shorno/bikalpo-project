...
export default function SuppliersPage() {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) =>
            orpc.warehouse.deleteSupplier.call({ id }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["warehouse", "suppliers"] });
        },
    });
                ...
                <div className="grid gap-3">
                    {suppliers.map((s: any) => (
                        ...
                        >
                            ...
                            {/* Financial info row */}
                            {(parseFloat(s.creditLimit || "0") > 0 || parseFloat(s.currentPayable || "0") > 0 || s.returnPackAgreement) && (
                                <div className="flex flex-wrap gap-3 mt-3 pt-3 border-t border-gray-100 text-xs">
                                    {parseFloat(s.creditLimit || "0") > 0 && (
                                        <span className="flex items-center gap-1 text-gray-500">
                                            <CreditCard size={11} />
                                            Credit Limit: ৳{parseFloat(s.creditLimit).toLocaleString()}
                                        </span>
                                    )}
