...
export default function WarehouseInventoryPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => orpc.warehouse.getMyInventory.call({ search, page: 1, limit: 200 }),
    });

    const updateMutation = useMutation({
        mutationFn: (data: { inventoryId: number; retailPrice?: string; availableQty?: string }) =>
    ...
    const totalStock = items.reduce((sum: number, i: any) => sum + Number(i.availableQty || 0), 0);
    const lowStockCount = items.filter((i: any) => Number(i.availableQty || 0) <= 5 && Number(i.availableQty || 0) > 0).length;
    const outOfStockCount = items.filter((i: any) => Number(i.availableQty || 0) === 0).length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Manage your warehouse stock levels and pricing</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="bg-white border border-amber-200 rounded-lg px-4 py-3 bg-amber-50/50">
                    <div className="text-xs text-amber-600 uppercase font-semibold">Low Stock</div>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center border rounded-lg bg-gray-50/50">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                        Go to <a href="/warehouse/dashboard/products" className="text-amber-600 underline font-medium">Products</a> to add products to your inventory.
                    </p>
                </div>
            ) : (
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50 border-b">
                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Product / Variant</th>
                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Category</th>
                                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Base Price</th>
                                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Retail Price</th>
                                    ...
                                    <tr key={item.id} className="border-b last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            {isEditing ? (
                                                ...
                                                <span className={`font-medium ${isOutOfStock ? "text-red-500" : isLowStock ? "text-amber-600" : "text-gray-900"}`}>
                                                    {qty.toLocaleString()}
                                                </span>
                                            )}
                                        </td>
