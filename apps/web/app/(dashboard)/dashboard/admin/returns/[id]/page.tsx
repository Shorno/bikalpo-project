...
export default async function AdminReturnDetailPage({
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-6 p-6">
      {/* Header */}
      <div className="flex items-start gap-2 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] sm:text-sm text-muted-foreground">
            <Link
              href={`${ADMIN_BASE}/orders/${returnData.orderId}`}
      ...
      {/* Process Return - Approve / Decline (only when pending) */}
      {returnData.status === "pending" && (
        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <p className="font-medium">Process this return</p>
              <p className="text-sm text-muted-foreground">
                Approve to refund and optionally restock; decline to reject the
                return.
              </p>
            </div>
            <ProcessReturnDialog
              returnRequest={returnData}
              trigger={<Button>Approve / Decline</Button>}
            />
          </CardContent>
        </Card>
      )}
      ...
      {/* Return Items (reason & photo from deliveryman) */}
      <Card className="p-0">
        <CardContent className="p-3 sm:p-4">
          <p className="text-xs font-semibold text-muted-foreground mb-2 sm:mb-3 uppercase tracking-wide">
      ...
      {/* Notes & Admin Notes */}
      {(returnData.notes || returnData.adminNotes) && (
        <Card className="p-0">
          <CardContent className="p-3 sm:p-4 space-y-3">
            {returnData.notes && (
              <div>
                <p className="text-xs font-semibold text-muted-foreground mb-1 uppercase tracking-wide">
                  Notes (from deliveryman)
                </p>
            ...
            {returnData.adminNotes && (
              <div className={returnData.notes ? "pt-2 border-t" : ""}>
                <p className="text-xs font-semibold text-muted-foreground mb-1 uppercase tracking-wide">
                  Admin Notes
                </p>
                <p className="text-sm whitespace-pre-wrap">
      ...
      {returnData.processor && (
        <Card className="p-0 bg-muted/30">
          <CardContent className="p-3 sm:p-4">
            ...
            {returnData.restocked != null && returnData.restocked > 0 && (
