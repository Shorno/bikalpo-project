...
const behaviourLabels: Record<string, { label: string; emoji: string; desc: string }> = {
    auto_break: { label: "Auto Break", emoji: "📦→📦📦", desc: "Carton breaks into packs" },
    loose_convert: { label: "Loose Convert", emoji: "🏷→⚖️", desc: "Sack converts to weight" },
    fixed_pack: { label: "Fixed Pack", emoji: "📦→📦", desc: "Same pack from warehouse to shop" },
};
...
export default function TypesPage() {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) =>
            orpc.adminProductType.delete.call({ id }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["adminProductType"] });
        },
    });
                ...
                >
                    ...
                    <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <Toggle label="Brand" checked={form.enableBrand} onChange={(v) => setForm({ ...form, enableBrand: v })} />
                            <Toggle label="Color" checked={form.enableColor} onChange={(v) => setForm({ ...form, enableColor: v })} />
                            <Toggle label="Size" checked={form.enableSize} onChange={(v) => setForm({ ...form, enableSize: v })} />
                            <Toggle label="Design" checked={form.enableDesign} onChange={(v) => setForm({ ...form, enableDesign: v })} />
                            <Toggle label="Variant" checked={form.enableVariant} onChange={(v) => setForm({ ...form, enableVariant: v })} />
                        </div>
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {(["auto_break", "loose_convert", "fixed_pack"] as const).map((behaviour) => {
                                const b = behaviourLabels[behaviour]!;
                                return (
                                    <button
                                        key={behaviour}
                                        type="button"
                                        onClick={() => setForm({ ...form, inventoryBehaviour: behaviour })}
                                    ...
                                    >
                                        <div className="text-sm font-medium text-gray-800">{b.emoji} {b.label}</div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <button type="button" onClick={resetForm} className="px-4 py-2 border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 text-sm">
                            Cancel
                        </button>
                    </div>
                </form>
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-2">
                                            {t.enableBrand && <span className="text-[10px] px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded">Brand</span>}
                                            {t.enableColor && <span className="text-[10px] px-1.5 py-0.5 bg-purple-50 text-purple-600 rounded">Color</span>}
                                            {t.enableSize && <span className="text-[10px] px-1.5 py-0.5 bg-orange-50 text-orange-600 rounded">Size</span>}
                                            {t.enableDesign && <span className="text-[10px] px-1.5 py-0.5 bg-pink-50 text-pink-600 rounded">Design</span>}
                                            {t.enableVariant && <span className="text-[10px] px-1.5 py-0.5 bg-teal-50 text-teal-600 rounded">Variant</span>}
                                    ...
                                    <div className="flex items-center gap-1 ml-4">
                                        ...
                                        <button
                                            onClick={() => { if (confirm("Delete this type?")) deleteMutation.mutate(t.id); }}
