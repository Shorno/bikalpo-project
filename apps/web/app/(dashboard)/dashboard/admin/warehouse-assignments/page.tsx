...
export default function WarehouseAssignmentsPage() {
    ...
    const { data: assignmentData, isLoading: loadingAssignments } = useQuery({
        queryKey: ["adminWarehouseAssignment", "getAssignments", selectedWarehouseId],
        queryFn: () =>
    ...
    const assignMutation = useMutation({
        mutationFn: (data: { warehouseId: string; categoryId: number; subcategoryId?: number }) =>
            orpc.adminWarehouseAssignment.assign.call(data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["adminWarehouseAssignment", "getAssignments"] });
        },
    });
        ...
        <div className="p-6 max-w-6xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    Assign product categories to warehouses — they can only add products from assigned categories
                </p>
            </div>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-4">
                    <h2 className="text-xs font-semibold text-gray-400 uppercase mb-2">Warehouses</h2>
                    {loadingWarehouses ? (
                        <div className="text-gray-400 text-sm py-4">Loading...</div>
                    ) : warehouses.length === 0 ? (
                        <div className="text-gray-400 text-sm py-4">No warehouses registered yet</div>
                    ) : (
                        <div className="space-y-1">
                            {warehouses.map((w: any) => (
                                <button
                                    key={w.id}
                                    onClick={() => { setSelectedWarehouseId(w.id); setShowPicker(false); }}
                ...
                <div className="col-span-8">
                    {!selectedWarehouseId ? (
                        <div className="text-center py-20 bg-white rounded-xl border border-gray-200">
                            ...
                            <p className="text-sm text-gray-400 mt-1">Choose a warehouse from the left to manage its category assignments</p>
                        </div>
                    ) : (
                        <div>
                                        ...
                                        return (
                                            <div key={type.id} className="mb-2">
                                                ...
                                                >
                                                    ...
                                                    <span className="text-[10px] text-gray-400 ml-auto">{typeCats.length} categories</span>
                                                </button>
                                                            ...
                                                            return (
                                                                <div key={cat.id}>
                                                                    <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50">
                                                                        {subs.length > 0 && (
                                                                            <button onClick={() => toggleCategory(cat.id)} className="p-0.5 hover:bg-gray-200 rounded">
                                                                        ...
                                                                        {catAssigned ? (
                                                                            <span className="text-[10px] px-1.5 py-0.5 bg-emerald-100 text-emerald-600 rounded">ASSIGNED</span>
                                                                        ) : (
                                                                            <button
                                                                    ...
                                                                    {catExpanded && subs.length > 0 && (
                                                                        <div className="ml-8 space-y-0.5">
                                                                            {subs.map((sub: any) => {
                                                                                const subAssigned = isAssigned(cat.id, sub.id);
                                                                                return (
                                                                                    <div key={sub.id} className="flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-50">
                                                                                        <span className="w-3 h-px bg-gray-300" />
                                                                                        <span className="text-xs text-gray-600 flex-1">{sub.name}</span>
                                                                                        {subAssigned ? (
                                                                                            <span className="text-[9px] px-1 py-0.5 bg-emerald-100 text-emerald-600 rounded">ASSIGNED</span>
                                                                                        ) : (
                                                                                            <button
                                                                                                onClick={() => handleAssign(cat.id, sub.id)}
                                                                                                disabled={assignMutation.isPending}
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <div className="ml-6 mt-1 space-y-1">
                                                {(categoriesByType.get(null) ?? []).map((cat: any) => {
                                                    const catAssigned = isAssigned(cat.id);
                                                    return (
                                                        <div key={cat.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50">
                                                            <Tag size={12} className="text-gray-500" />
                                                            <span className="text-sm text-gray-700 flex-1">{cat.name}</span>
                                                            {catAssigned ? (
                                                                <span className="text-[10px] px-1.5 py-0.5 bg-emerald-100 text-emerald-600 rounded">ASSIGNED</span>
                            ...
                            {loadingAssignments ? (
                                ...
                                <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                                    ...
                                    <p className="text-gray-500 text-sm font-medium">No categories assigned</p>
                                    <p className="text-xs text-gray-400 mt-1">
                                        Click "Assign Category" to add categories this warehouse can use
                                    </p>
                                </div>
                            ) : (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                                                    ...
                                                    <Tag size={14} className={a.subcategory ? "text-blue-600" : "text-emerald-600"} />
                                                </div>
                                                <div>
