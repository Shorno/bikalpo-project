      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg border shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Stock is auto-converted from wholesale (TRADE) to retail (RETAIL)
            when B2B orders are delivered.
          </p>
          <Button
          ...
          <Table>
            ...
            <TableBody>
              {items.map((item) => {
                ...
                const img = prod?.images?.[0]?.imageUrl || prod?.image;
...
function WarehouseUrlModal({ onClose }: { onClose: () => void }) {
  ...
  function handlePreview() {
    ...
    if (!slug) {
      setError(
        "Please enter a valid warehouse URL or slug. Example: /warehouse/my-warehouse",
      );
      return;
    }
    setParsedSlug(slug);
