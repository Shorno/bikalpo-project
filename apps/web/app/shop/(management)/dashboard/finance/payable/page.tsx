...
export default function PayablePage() {
  ...
  const { data: summary, isLoading } = useQuery(
    orpc.supplierPayment.getPayableSummary.queryOptions({}),
  );

  // Get supplier payment ledger
  const { data: ledgerEntries } = useQuery({
    ...orpc.supplierPayment.getSupplierLedger.queryOptions({
      input: { supplierId: viewLedgerId! },
    }),
  ...
  const payMutation = useMutation(
    orpc.supplierPayment.paySupplier.mutationOptions({
      onSuccess: (result) => {
        ...
        setPayForm({ amount: "", paymentMethod: "cash", referenceNo: "", note: "" });
      },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Outstanding credit balances with suppliers. Only supplier purchases create payables.
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-orange-700 mt-1">৳{Number(totalPayable).toLocaleString()}</p>
          </div>
          <div className="text-xs text-gray-500">{suppliers.length} suppliers with dues</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-500">No outstanding payables</p>
            <p className="text-xs text-gray-400 mt-1">Credit purchases will appear here</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {suppliers.map((sup: any) => (
                <tr key={sup.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right text-gray-600">৳{Number(sup.creditLimit).toLocaleString()}</td>
                  <td className="px-4 py-3 text-right">
                    <span className="font-semibold text-orange-600">৳{Number(sup.currentPayable).toLocaleString()}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => { setPayingSupplierId(sup.id); setPayingSupplierName(sup.name); }}
                        className="px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"
                      >
                        Pay
                      </button>
                      <button
                        onClick={() => setViewLedgerId(viewLedgerId === sup.id ? null : sup.id)}
      ...
      {/* Payment Ledger (expanded inline) */}
      {viewLedgerId && ledgerEntries && (ledgerEntries as any[]).length > 0 && (
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
          <div className="p-4 border-b bg-gray-50 flex items-center justify-between">
            ...
            <button onClick={() => setViewLedgerId(null)} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {(ledgerEntries as any[]).map((entry: any) => (
                <tr key={entry.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 text-gray-600">{new Date(entry.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-2 text-gray-700">{entry.description}</td>
                  <td className="px-4 py-2 text-right font-medium text-green-600">৳{Number(entry.amount).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b">
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                ...
                Pay Supplier: {payingSupplierName}
              </h2>
              <button onClick={() => setPayingSupplierId(null)} className="p-1 hover:bg-gray-100 rounded-lg"><X size={18} /></button>
            </div>
            <form onSubmit={handlePay} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Amount (৳) *</label>
                <input
                  type="number" step="0.01" min="0" value={payForm.amount}
              ...
              <div>
                ...
                <select value={payForm.paymentMethod} onChange={(e) => setPayForm({ ...payForm, paymentMethod: e.target.value as any })}
                  className="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <input type="text" value={payForm.referenceNo} onChange={(e) => setPayForm({ ...payForm, referenceNo: e.target.value })}
                  className="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
              ...
              <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg border border-amber-200">
                ...
                <p className="text-xs text-amber-700">Payment will reduce supplier outstanding and deduct from cash/bank balance.</p>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setPayingSupplierId(null)}
