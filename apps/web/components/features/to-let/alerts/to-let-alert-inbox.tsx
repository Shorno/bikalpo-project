  ...
  return (
    <section aria-labelledby="alert-inbox-heading" className="space-y-4">
      ...
      {isPending || inbox.isLoading ? (
        <div role="status" className="rounded-xl border border-border bg-card p-6 text-sm text-muted-foreground">Loading matching rentals…</div>
      ) : inbox.isError ? (
                ...
                {listing ? (
                  <>
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 sm:px-5">
                      <div>
                        <p className="text-xs text-muted-foreground">Received on {alertDate(item.createdAt, true)}</p>
                        <p className="mt-1 text-sm font-semibold text-foreground">{item.readAt ? "Rental match" : "New rental match"}</p>
                      </div>
                      ...
                      <div className="min-w-0 space-y-4 p-4 sm:p-5">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="min-w-0 flex-1 basis-40">
                            ...
                            <h3 className="mt-1 break-words text-lg font-semibold text-foreground">{listing.title}</h3>
                            <p className="mt-1 break-words text-sm text-muted-foreground">{listing.propertyName} · {listing.unitName}</p>
                          </div>
                          <div className="sm:text-right">
                            ...
                            {listing.monthlyRent !== null && <p className="text-xs text-muted-foreground">monthly rent</p>}
                          </div>
                        </div>
                        <p className="flex items-start gap-2 text-sm text-muted-foreground"><MapPin className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-hidden="true" />{listing.location}</p>
                        <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm tabular-nums text-muted-foreground">
                          <span className="inline-flex items-center gap-1.5"><BedDouble className="size-4 text-muted-foreground" aria-hidden="true" />{listing.bedrooms} beds</span>
                          <span className="inline-flex items-center gap-1.5"><Bath className="size-4 text-muted-foreground" aria-hidden="true" />{listing.bathrooms} baths</span>
                          <span className="inline-flex items-center gap-1.5"><Ruler className="size-4 text-muted-foreground" aria-hidden="true" />{listing.sizeSqFt.toLocaleString("en-BD")} sq ft</span>
                        </div>
                        {!!listing.facilities?.length && <p className="text-sm text-muted-foreground"><span className="font-medium text-foreground">Facilities:</span> {listing.facilities.slice(0, 3).join(" · ")}{listing.facilities.length > 3 ? ` +${listing.facilities.length - 3} more` : ""}</p>}
                        <div className="rounded-lg bg-muted/30 p-3 text-sm"><span className="text-muted-foreground">Available from:</span> <span className="font-medium text-foreground">{listing.availableFrom ? alertDate(listing.availableFrom) : "Not specified"}</span></div>
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-3">
                          <p className="text-xs text-muted-foreground">Matches your category, location & minimum size</p>
                          {item.readAt ? <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><Check className="size-3.5" aria-hidden="true" />Read</span> : <Button size="sm" variant="outline" disabled={markRead.isPending} onClick={() => markRead.mutate({ notificationIds: [item.id] })}><Check className="size-3.5" />Mark as read</Button>}
                        </div>
                      </div>
                    </div>
                  </>
