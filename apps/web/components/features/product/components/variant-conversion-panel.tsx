...
function variantLabel(v?: ProductVariant | null) {
    if (!v) return "Unknown";
    const type = v.variantType === "trade" ? "Trade" : v.variantType === "retail" ? "Retail" : "";
...
export function VariantConversionPanel({
    ...
    const { data: rules = [] } = useQuery<ConversionRule[]>({
        ...
        queryFn: () => {
            ...
            return client.adminProductVariant.listConversionRules({}).then((allRules: ConversionRule[]) =>
                allRules.filter(
                    (r) =>
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["conversion-rules", productId] });
    ...
    const hasTypedVariants = tradeVariants.length > 0 && retailVariants.length > 0;
                ...
                <CardContent>
                    {!hasTypedVariants ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-6 text-center border rounded-lg bg-muted/30">
                            <Link2 className="size-8 text-muted-foreground mb-2" />
                            <p className="text-sm text-muted-foreground">
                                No conversion rules yet. Add one to link Trade → Retail variants.
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {rules.map((rule) => {
                                const from = variants.find((v) => v.id === rule.fromVariantId) ?? rule.fromVariant;
                                const to = variants.find((v) => v.id === rule.toVariantId) ?? rule.toVariant;
                                return (
                                    <div
                                        key={rule.id}
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This conversion rule will be deleted. Inventory auto-conversion for
                            this link will stop.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
