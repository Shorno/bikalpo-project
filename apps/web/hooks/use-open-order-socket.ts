...
const OPEN_ORDER_UPDATE_EVENTS = [
  "open-order:offer-received",
  "open-order:offer-updated",
  "open-order:offer-withdrawn",
  "open-order:offer-window-closed",
  "open-order:accepted",
  "open-order:not-selected",
  "open-order:cancelled",
  "open-order:expired",
  "open-order:no-offers",
] as const;

/** Realtime is an accelerator; callers still keep polling as a correctness fallback. */
export function useOpenOrderUpdates(
  orderId: number | null,
  onUpdate: () => void,
) {
  const { socket, isConnected } = useSocket();
  ...
  useEffect(() => {
    ...
    for (const event of OPEN_ORDER_UPDATE_EVENTS) socket.on(event, handler);
    return () => {
      socket.emit("leave-order", orderId);
      for (const event of OPEN_ORDER_UPDATE_EVENTS) socket.off(event, handler);
    };
...
export function useShopOpenOrderUpdates(onUpdate: () => void) {
  const { socket, isConnected } = useSocket();
  const callbackRef = useRef(onUpdate);
  callbackRef.current = onUpdate;
  ...
  useEffect(() => {
    ...
    const handler = () => callbackRef.current();
    const events = [
      "open-order:new-request",
      ...OPEN_ORDER_UPDATE_EVENTS,
    ] as const;
